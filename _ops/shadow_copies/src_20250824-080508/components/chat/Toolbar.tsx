...
type Props = {
  onPrev?: () => void;
  onNext?: () => void;
  mutedOutput?: boolean;          // 🔊 app sound
  onToggleOutput?: () => void;
  mutedMic?: boolean;             // 🎤 local mic
  onToggleMic?: () => void;
  liked?: boolean;                // ❤️ like toggle
  onToggleLike?: () => void;
    ...
    <div className={`w-full bg-black/30 backdrop-blur-md border-t border-white/10 ${className}`}>
      <div className="flex items-center gap-2 px-2 py-2">
        ...
        <div className="flex-0">
          <Btn aria="Previous" onClick={onPrev} big>
            <span aria-hidden>◀</span><span className="hidden sm:inline"> Prev</span>
          </Btn>
        </div>

        {/* CENTER: scrollable action group */}
        <div className="flex-1 overflow-x-auto no-scrollbar">
          <div className="min-w-max flex items-center justify-center gap-2">
            <Btn aria={mutedOutput? "Unmute sound":"Mute sound"} onClick={onToggleOutput}>
              <span aria-hidden>{mutedOutput ? "🔇" : "🔊"}</span>
              <span className="hidden sm:inline">{mutedOutput ? "Muted" : "Sound"}</span>
            </Btn>

            <Btn aria={mutedMic? "Unmute mic":"Mute mic"} onClick={onToggleMic}>
              <span aria-hidden>{mutedMic ? "🎙️✖️" : "🎙️"}</span>
              <span className="hidden sm:inline">{mutedMic ? "Mic off" : "Mic"}</span>
            </Btn>
            ...
            <Btn aria="Report" onClick={onReport}>
              <span aria-hidden>🚩</span><span className="hidden sm:inline">Report</span>
            </Btn>
          </div>
        </div>
        ...
        {/* RIGHT: Next */}
        <div className="flex-0">
          <Btn aria="Next" onClick={onNext} big>
