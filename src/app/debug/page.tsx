...
export default function DebugPage() {
  ...
  const sock = useRef<SocketT | null>(null);
  ...
  useEffect(() => {
    ...
    const s = makeSocket(base, { path: '/socket.io', transports: ['polling'], upgrade: false, query: { userId }, forceNew: true });
    sock.current = s;

    s.on('connect', () => { setConnected(true); log('connect'); });
    s.on('connect_error', (err: unknown) => {
      const msg = (err instanceof Error) ? err.message
        : (typeof err === 'object' && err && (err as any).message) ? String((err as any).message)
        : String(err);
      log(`connect_error: ${msg}`);
    });
