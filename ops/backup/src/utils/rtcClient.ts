...
export class RTCClient {
  ...
  async start({ socketUrl, roomId, localEl, remoteEl }: Cfg) {
    ...
    // 1) socket
    this.socket = makeSocket(socketUrl, { path: '/socket.io', reconnection: true });
    ...
    this.socket!.on('rtc:signal', async ({ from, payload  }: any) => {
      if (!this.pc) return;
      if (payload.type === 'offer') {
        ...
        try {
          await this.pc.addIceCandidate(new RTCIceCandidate((payload.candidate) as RTCIceCandidateInit));
        } catch {}
      }
    });
