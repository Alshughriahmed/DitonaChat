...
export default function RoomPage({ params }: { params: { room: string } }) {
  ...
  async function start() {
    try {
      ...
      // إنشاء Socket (same-origin) بدون with_creds
      const s: SocketT = makeSocket(baseUrl, {
        path: '/socket.io',
      ...
      s.on('rtc:roomSize', (size: number) => {
        push('roomSize=' + size);
        if (size === 1) isCaller = true;
      });
      ...
      s.on('rtc:offer', async ({ sdp }: OfferPayload) => {
        push('recv offer');
      ...
      s.on('rtc:answer', async ({ sdp }: AnswerPayload) => {
        push('recv answer');
        await pc.setRemoteDescription(new RTCSessionDescription(sdp));
        setStatus('in-call');
      });

      s.on('rtc:candidate', async ({ candidate }: CandidatePayload) => {
        try {
          await pc.addIceCandidate(new RTCIceCandidate(candidate));
          push('recv candidate');
        } catch (e: any) {
          push('candidate error ' + e.message);
        }
      });
