...
type Msg = { id: string; from: "me" | "peer"; text: string; ts?: number };
...
export default function ChatPage(){
  React.useEffect(() => {
    ...
    const update = () => {
      ...
      document.documentElement.style.setProperty('--kb-pad', kb + 'px');
    };
    update();
  ...
  // === chat state (moved inside component) ===
  const [dockMode, setDockMode] = React.useState<"latest"|"history">("latest");
...
setPeer(p => ({...p, likes})); }, [likes]);
  ...
  return (
    <div className="relative min-h-[100dvh] w-full overflow-hidden text-white bg-gradient-to-b from-slate-900 via-slate-900 to-black h-[100dvh] grid grid-rows-[minmax(0,1fr)_minmax(0,1fr)]" style={{ touchAction: 'pan-x', }}>
...
<ChatMessages msgs={msgs}/>
      ...
      <div className="row-start-3 legacy-shelf">

{/* === Chat messages dock (transparent) === */}
<div className="fixed left-0 right-0 z-20 pointer-events-none"
     style={{ bottom: 'calc(var(--tb-h) + var(--kb-pad) + var(--safe-b))' }}>
  <div className="pointer-events-auto">
    <ChatMessages
      items={[
        { id: "m1", from: "peer", text: "Hello 👋" },
        { id: "m2", from: "me",   text: "Hi! This is a test." },
        { id: "m3", from: "peer", text: "All good — transparent dock." }
      ]}
      mode="latest"
    />
  </div>
        ...
        <Toolbar
          ...
          onToggleChat={() => setComposerOpen(v=>!v)}
      ...
      <ChatComposer open={true} onSend={send}/>
    </div>
    </div>
